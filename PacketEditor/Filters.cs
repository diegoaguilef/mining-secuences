...
namespace PacketEditor
{
    public partial class Filters : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            EditFilter frmChReplay = new EditFilter(dr, sinfo);
            if (this.TopMost == true)
                frmChReplay.TopMost = true;
            frmChReplay.ShowDialog();
            if (frmChReplay.retval == true)
            {
                string funs = "";
                if (dtFilters.Rows.Find(dr["id"].ToString()) != null)
                {
                    int x;
                    for (x = 2; dtFilters.Rows.Find(dr["id"].ToString() + x.ToString()) != null; x++) { }
                    dr["id"] = dr["id"].ToString() + x.ToString();
                }
        ...
        private void dgridFilters_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if ((e.ColumnIndex != 0) && (e.RowIndex != -1))
            {
                ...
                {
                    string funs = "";
                    dgridFilters.Rows[e.RowIndex].Cells["name"].Value = dr["id"].ToString();
                    dgridFilters.Rows[e.RowIndex].Cells["enabled"].Value = dr["enabled"];
                    foreach (byte f in (byte[])dr["MsgFunction"])
                    {
                        funs += sinfo.msg(f) + " ";
                    }
                    ...
                    if (funs != string.Empty)
                    {
                        dgridFilters.Rows[e.RowIndex].Cells["function"].Value = funs.TrimEnd();
                    }
                }
            }
        }
